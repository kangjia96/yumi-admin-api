...
(
    ...
    keep_alive  int          default 0   null comment '是否缓存（0缓存 1不缓存）',
    visible     tinyint(1)   default 0   null comment '菜单状态（0显示 1隐藏）',
    status      tinyint(1)   default 0   null comment '菜单状态（0正常 1停用）',
    create_time datetime                 null comment '创建时间',
    update_time datetime                 null comment '更新时间'
    );
